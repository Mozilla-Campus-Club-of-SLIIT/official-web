...
export default function JoinUsPage() {
  ...
  const validate = useCallback((): FieldErrors => {
    ...
    if (!fullName.trim()) nextErrors.fullName = "Full name is required";
      ...
      nextErrors.fullName = "Only letters and spaces";
    ...
    // WhatsApp
  if (!/^[0-9]{1,3}$/.test(whatsappCountryDigits)) nextErrors.whatsappCountry = "Country code 1-3 digits";
    if (!/^[0-9]{9}$/.test(whatsappNumber))
      nextErrors.whatsappNumber = "Exactly 9 digits";
  ...
  const baseInput = "w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#EA7B2C] transition-colors";
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="space-y-5" noValidate>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setFullName(val);
              // live validation
              setErrors((prev) => ({ ...prev, fullName: !val.trim() ? "Full name is required" : !/^[A-Za-z ]+$/.test(val.trim()) ? "Only letters and spaces" : undefined }));
            }}
        ...
        <div>
          ...
          {errors.studentId && touched.studentId && (
            <p id="studentId-error" className="mt-1 text-xs text-red-600">{errors.studentId}</p>
          )}
        </div>
        ...
        <div>
          ...
          <div className="flex gap-3">
            <div className="flex items-stretch">
              <span className="px-2 py-2 border border-r-0 rounded-l bg-gray-50 text-sm select-none">+</span>
              <input
                ...
                aria-describedby={errors.whatsappCountry && touched.whatsappCountry ? "whatsappCountry-error" : undefined}
              />
            </div>
            <input
              name="whatsappNumber"
              ...
              aria-describedby={errors.whatsappNumber && touched.whatsappNumber ? "whatsappNumber-error" : undefined}
            />
          </div>
  ...
  <option value="998 - Uzbekistan" />
          ...
          {errors.whatsappCountry && touched.whatsappCountry && (
            <p id="whatsappCountry-error" className="mt-1 text-xs text-red-600">{errors.whatsappCountry}</p>
          )}
          {errors.whatsappNumber && touched.whatsappNumber && (
            <p id="whatsappNumber-error" className="mt-1 text-xs text-red-600">{errors.whatsappNumber}</p>
          )}
        </div>
        ...
        {/* Preferred Teams */}
        <div>
          <fieldset className={`border rounded p-3 ${errors.preferredTeam && touched.preferredTeam ? 'border-red-500' : 'border-gray-300'}`}>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              {preferredTeamOptions.map((team) => {
                ...
                return (
                  <label key={team} className="flex items-center gap-2 text-sm cursor-pointer select-none">
                    <input
                      type="checkbox"
                      value={team}
                      checked={checked}
